...
import "./lib/IERC20Monsterra.sol";
...
contract Monsterra is Initializable, UUPSUpgradeable, OwnableUpgradeable, SignatureUtils, ReentrancyGuardUpgradeable{
   ...
   */
   ...
   /* ====================== MAIN FUNCTIONS ====================== */
   /**
    * @dev user will burn their cur accepted token in order to recevie des accepted token from the destination chain
   ...
   function burn(
      ...
      require(isTransactionExisted(transactionId_) != true, "Burn: Transaction existed");
      _validateSwapData(curToken_, desToken_, amount_);

      // Burn token
      require(IERC20Monsterra(curToken_).allowance(msg.sender, address(this)) >= amount_, "Mint: Transfer amount exceeds allowance");
      IERC20Monsterra(curToken_).burnFrom(
         msg.sender,
         amount_
      );
      ...
      emit SwapEvent(transactionId_, curToken_, desToken_, msg.sender, desUser_, amount_, BURN);
   }
   ...
   function mint(
      ...
      IERC20Monsterra(curToken_).mint(
         msg.sender,
         amount_
      );
      ...
      swapDatas[transactionId_] = swapData;
      emit SwapEvent(transactionId_, curToken_, desToken_, msg.sender, desUser_, amount_, MINT);
   }
